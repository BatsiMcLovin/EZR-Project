...
float calcVisibility(vec3 position, float normal_weight)
{
...
vec2 tc[4];
tc[0] = vec2(1,0) * resolution;
tc[1] = vec2(0,1) * resolution;
tc[2] = vec2(-1,0) * resolution;
tc[3] = vec2(0,-1) * resolution;

int num_samples = numPoissonSamples;
float subtractor = 1.0/num_samples;
float spreadage = 1500 - 1000.0 * (normal_weight);
float bias = position.z + 0.00005;
 ...
 for (int i=0; i<num_samples; i++)
 {
   ...
   {
     v[0] = v[0] - weighted_subtractor;
   }

   if ( texture( shadowTex, tc[0] + poisson_tc ).x  < bias)
   {
     v[1] = v[1] - weighted_subtractor;
   }

   if ( texture( shadowTex, tc[1] + poisson_tc ).x  < bias)
   {
     v[2] = v[2] - weighted_subtractor;
   }
   if ( texture( shadowTex, tc[2] + poisson_tc ).x  < bias)
   {
     v[3] = v[3] - weighted_subtractor;
   }
   if ( texture( shadowTex, tc[3] + poisson_tc ).x  < bias)
   {
     v[4] = v[4] - weighted_subtractor;
   }
 }

 float p = 0.5f;

return v[0] * p + (v[1] + v[2] + v[3] + v[4]) * p * 0.25;
}
...
float visibility = calcVisibility(position.xyz, offset);
