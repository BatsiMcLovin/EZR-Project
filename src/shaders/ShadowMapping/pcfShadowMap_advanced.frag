...
vec4 pcf(vec3 position, vec3 position_w, float normal_weight)
{
...
int radius = int(pcfMaxRadius * normal_weight * 0.75 + (depth_weight) * pcfMaxRadius * 0.25);
...
for(int i = -radius; i <= radius; i++)
{
    for(int j = -radius; j <= radius; j++)
    {
        ...
        {
            hits++;
            totalDepth = max(totalDepth, abs(depth - bias));
        }
    }
}
...
return vec4(result, result, result, totalDepth);
return vec4(result, result, result, totalDepth / hits);
}
...
vec4 visibility = pcf(position.xyz, position_w, offset);
